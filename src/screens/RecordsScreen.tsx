  ...
  export default function RecordsScreen() {
    ...
    const handleTap = (p: PlayerStat) => {
      const playerId = idMap[p.name]
      if (playerId) navigation.navigate('PlayerProfile', { id: playerId })
      else setSelected(p)
    }
  ...
  setMatches(matchesData)
        ...
        ;(Array.isArray(pr)?pr:pr.data||[]).forEach((p:any) => { pm[p.name]=p.photoUrl||''; if (p._id) im[p.name]=p._id })
        setPhotoMap(pm)
        setIdMap(im)
        setLoading(false)
      }
      ...
      if (cat) return (
        <View style={styles.root}>
          ...
          <CategoryLeaderboard title={cat.label} icon={cat.icon} players={cat.players}
            ...
            asc={(cat as any).asc||false} onTap={handleTap} onBack={()=>setCategory(null)} />
