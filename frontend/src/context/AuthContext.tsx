...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [sessionSalt, setSessionSalt] = useState(() => Math.random().toString(36).substring(2, 9))
    ...
    const init = async () => {
      try {
        ...
        try {
          ...
          console.log('[AuthContext] init: getDeviceId failed:', e)
        }
        ...
        if (guestMode === 'true') {
          setIsGuest(true)
          const prefix = typeof did === 'string' && did.length >= 4 ? did.slice(0, 4).toUpperCase() : 'USER'
          setUser({ id: did || 'guest', name: `Guest-${prefix}` })
          return
        }
  ...
  const continueAsGuest = useCallback(async () => {
    ...
    try {
      ...
      console.log('[AuthContext] continueAsGuest: getDeviceId failed:', e)
    }
  ...
  return (
    <AuthContext.Provider value={{
      user, loading, isGuest, deviceId, sessionSalt,
