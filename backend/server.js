...
mongoose.connect(process.env.MONGO_URI || process.env.MONGODB_URI)
  .then(() => {
    ...
    // Drop unique index on deviceId if it exists to prevent duplicate key errors
    mongoose.connection.db.collection('users').dropIndex('deviceId_1')
      .then(() => console.log('✅ Unique index deviceId_1 dropped'))
      .catch(() => {}) // Ignore if index does not exist
